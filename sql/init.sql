...
CREATE TABLE IF NOT EXISTS pagos (
    ...
    observaciones TEXT,
    tipo_pago ENUM('primer_pago', 'cuota_mensual') DEFAULT 'primer_pago',
    FOREIGN KEY (id_usuario) REFERENCES Usuario(id_usuario) ON DELETE CASCADE,
    INDEX idx_usuario (id_usuario),
...
LEFT JOIN Cuotas_Mensuales cm ON 
    YEAR(rh.fecha) = cm.anio AND 
    MONTH(rh.fecha) = cm.mes AND
    rh.id_usuario = cm.id_usuario
LEFT JOIN Pagos_Cuotas pc ON cm.id_cuota = pc.id_cuota AND pc.estado_validacion = 'aprobado'
...
-- ==========================================
-- TRIGGER: Actualizar cuota al asignar vivienda (Usuario y Nucleo)
-- ==========================================
...
BEGIN
    ...
    IF NEW.activa = 1 THEN
        ...
        IF NEW.id_usuario IS NOT NULL THEN
            ...
            AND (
                (anio = YEAR(CURDATE()) AND mes >= MONTH(CURDATE())) OR
                anio > YEAR(CURDATE())
            );
        ...
        IF NEW.id_nucleo IS NOT NULL THEN
            ...
            INNER JOIN Usuario u ON cm.id_usuario = u.id_usuario
            SET 
                cm.id_vivienda = NEW.id_vivienda,
                ...
                cm.observaciones = CONCAT(
                    ...
                    'Vivienda asignada a nucleo el ', NOW()
                )
            ...
            INSERT INTO notificaciones (titulo, mensaje, tipo)
            VALUES (
                'Vivienda Asignada a tu Nucleo',
                'Se ha asignado una vivienda a tu nucleo familiar. Tus cuotas han sido actualizadas.',
                'exito'
            );
...
-- ==========================================
-- TRIGGER: Generar cuota automaticamente cuando un usuario es aceptado
-- ==========================================
...
BEGIN
    ...
    IF NEW.estado = 'aceptado' AND OLD.estado != 'aceptado' THEN
        ...
        IF NOT EXISTS (
            ...
            WHERE (av.id_usuario = NEW.id_usuario OR av.id_nucleo = NEW.id_nucleo)
            AND av.activa = 1
            LIMIT 1;
            ...
            INSERT INTO Cuotas_Mensuales 
                (id_usuario, id_vivienda, mes, anio, monto, monto_pendiente_anterior,
                 fecha_vencimiento, horas_requeridas, estado, pendiente_asignacion, observaciones)
            VALUES 
                (NEW.id_usuario, v_id_vivienda, v_mes_actual, v_anio_actual, v_monto_base, 0,
                 v_fecha_vencimiento, 84.00, 'pendiente', v_pendiente_asignacion,
                 IF(v_pendiente_asignacion = 1, 
                    CONCAT('Usuario aceptado el ', DATE_FORMAT(NOW(), '%d/%m/%Y'), '. Pendiente asignacion de vivienda.'),
                    CONCAT('Usuario aceptado el ', DATE_FORMAT(NOW(), '%d/%m/%Y'))
                 )
                );
            ...
            VALUES (
                ...
                CONCAT('Tu cuota de ', MONTHNAME(CURDATE()), ' ha sido generada. ',
                       IF(v_pendiente_asignacion = 1,
                          'Estas en espera de asignacion de vivienda.',
                          CONCAT('Monto: , CAST(v_monto_base AS CHAR), '. Horas requeridas: 84')
                       )
                ),
                'info'
            );
...
BEGIN
    ...
    read_loop: LOOP
        ...
        IF EXISTS (
            SELECT 1 FROM Cuotas_Mensuales
            WHERE id_usuario = v_id_usuario AND mes = p_mes AND anio = p_anio
        ) THEN
        ...
        VALUES 
            (v_id_usuario, v_id_vivienda, p_mes, p_anio, v_monto_base, v_deuda_anterior,
             v_fecha_vencimiento, 84.00, 'pendiente', v_pendiente_asignacion,
             IF(v_pendiente_asignacion = 1, 'Pendiente: Asignar vivienda', NULL));
    ...
    IF v_cuotas_sin_vivienda > 0 THEN
        ...
        VALUES (
            ...
            CONCAT('Hay ', v_cuotas_sin_vivienda, ' usuario(s) sin vivienda. Cuotas pendientes de asignacion.'),
            'urgente'
        );
...
STARTS CASE 
    WHEN DAY(CURDATE()) = 1 THEN 
        CONCAT(DATE_FORMAT(DATE_ADD(LAST_DAY(CURDATE()), INTERVAL 1 DAY), '%Y-%m-01'), ' 00:01:00')
    ELSE 
        CONCAT(DATE_FORMAT(DATE_ADD(LAST_DAY(CURDATE()), INTERVAL 1 DAY), '%Y-%m-01'), ' 00:01:00')
END
DO
