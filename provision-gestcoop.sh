...
ENV_FILE_PATH="${ENV_FILE_PATH:-/.env}" # Path al .env de producción
...
update_and_install() {
	...
	ubuntu | debian)
		...
		$SUDO apt-get install -y --allowerasing curl git wget ca-certificates gnupg lsb-release unzip netcat-openbsd
		;;
	centos | rhel | rocky)
		$SUDO dnf install -y --allowerasing curl git wget ca-certificates unzip nc procps-ng
		;;
	fedora)
		$SUDO dnf install -y --allowerasing curl git wget ca-certificates unzip nc procps-ng
		;;
	...
	if [[ "$OS" == "ubuntu" || "$OS" == "debian" ]]; then
      ...
      $(lsb_release -cs) stable" | $SUDO tee /etc/apt/sources.list.d/docker.list >/dev/null
		...
		if is_systemd_available; then
			log "Habilitando y arrancando Docker via systemd..."
			$SUDO systemctl enable --now docker
		else
			log "Systemd no disponible. Intentando iniciar Docker manualmente..."
...
setup_project() {
	...
	if [ -n "$SUDO" ]; then
		$SUDO $dc -f "$DC_FILE" build --no-cache || $SUDO $dc -f "$DC_FILE" build
		$SUDO $dc -f "$DC_FILE" up -d
	else
		$dc -f "$DC_FILE" build --no-cache || $dc -f "$DC_FILE" build
		$dc -f "$DC_FILE" up -d
	fi
	...
	(
		...
		echo "0 2 * * * bash $(pwd)/scripts/backup.sh >> $(pwd)/logs/backup.log 2>&1"
	) | crontab -
